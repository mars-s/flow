...
//! Flow's routing user driver keeps automatic checks in the sidebar, but
...
//! itself with a production build. `FLOW_PREVIEW_UPDATE=1` fakes only the
//! automatic sidebar result while retaining the real Sparkle flow for the
//! Check for Updates menu; `FLOW_FORCE_UPDATER=1` exercises everything for
...
/// The compact state rendered by Flow. Update details remain owned by
...
mod macos {
    ...
    struct UserDriverIvars {
        ...
        /// use Sparkle's own windows. Scheduled checks stay inside Flow.
    ...
    impl Updater {
        ...
        pub fn init() -> Option<Self> {
            let preview = cfg!(debug_assertions)
                && std::env::var_os("FLOW_PREVIEW_UPDATE").is_some_and(|value| value == "1");
            let forced = std::env::var_os("FLOW_FORCE_UPDATER").is_some_and(|value| value == "1");
            ...
            if handle.is_null() {
                ...
                eprintln!("Flow updater: failed to load Sparkle: {reason}");
                return None;
            }
            ...
            if !started {
                eprintln!("Flow updater: Sparkle rejected its updater configuration");
                return None;
            }
    ...
    /// (Contents/MacOS/Flow → Contents/Frameworks/Sparkle.framework/Sparkle).
