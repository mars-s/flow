...
// Lives in the `flow-data` crate now, not this one — same reasoning as
// `db`/`parse` (`src/lib.rs`'s own `use flow_data::…` re-exports): none of
// this ever depended on `gpui`, so it's reusable by any frontend
// (`wayfinder/tickets/migrate-to-tauri.md`). Re-exported here so every
// existing `crate::platform::calendar_…`/`CalendarAuth`/etc. reference in
// the app resolves unchanged. `open_calendar_privacy_pane` below is the one
// calendar-adjacent function that stayed — it's an `objc2-app-kit` deep
// link, not calendar data, and wasn't worth pulling that dependency into
// flow-data for yet.
pub use flow_data::calendar::{
    CalendarAuth, CalendarEvent, CalendarInfo, calendar_authorization_status,
    calendar_events_between, calendar_list, calendar_request_access, local_midnight,
};
