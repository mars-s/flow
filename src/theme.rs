...
impl Theme {
    pub fn current(cx: &App) -> Self {
        if cx.has_global::<ActiveFlowTheme>() {
            cx.global::<ActiveFlowTheme>().0
        } else {
            Self::dark()
        }
    }
...
#[derive(Clone, Copy)]
struct ActiveFlowTheme(Theme);

impl Global for ActiveFlowTheme {}
...
fn set_active_theme(theme: Theme, cx: &mut App) {
    cx.set_global(ActiveFlowTheme(theme));
}
