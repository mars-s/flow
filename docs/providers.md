...
How Flow talks to each coding agent: the process it launches, the wire protocol
...
polled off disk, and the one Flow generates itself — is in
[titles.md](titles.md).
...
`Flow::runtimes` keyed by session id. Runtimes are per session, not per view:
...
([src/opencode_session.rs](../src/opencode_session.rs)). Flow quitting without
...
Codex can send Flow requests (approvals) and Flow answers them by id. Three
...
2. `initialized`.
3. `skills/extraRoots/set` when Computer Use is on, so Flow's bundled skill is
...
per-peer, the reader only treats method-less messages as replies to Flow's own
...
**Computer Use** — `-c mcp_servers.flow_js_repl.command=…` registers Flow's
QuickJS MCP server, with several `-c` flags disabling Codex's own external
computer-use plugin/MCP/skill so only Flow's `js` / `js_reset` surface is
visible.
...
Flow stamps each request with a string id (`flow-<n>`) and Pi answers with
...
| `extension_ui_request` | auto-cancelled — Flow has no UI for extension prompts |
...
**Computer Use** — `--extension <flow pi extension>` and `--skill <SKILL.md>`,
...
| `user` with `isReplay: true` | ignored — Flow's own prompt echoed by `--replay-user-messages` |
| `result` | `TurnFinished` |
...
`permission_suggestions`. Flow answers with a `control_response` whose result is
...
**Native checkpoints** — after each turn Flow reads Claude's own transcript at
...
with fresh uuids; the returned id map is applied to Flow's retained turns.
...
permission request on its stream; its rules live in `amp permissions`, so Flow
...
**Branch** — `amp threads export <id>` dumps the thread, Flow keeps the retained
...
(`FLOW_AMP_BRANCH_CONTEXT_V1`). Forking a thread that was itself seeded this way
...
([src/driver/opencode.rs](../src/driver/opencode.rs)). Flow already started this
...
only under `/api`, and since this server is Flow's alone, filtering by
...
to. Alongside Codex's app-server, this is the only transport where Flow's
...
capability, since Flow does not proxy the agent's file or terminal access — an
...
whole basis for the user's decision. Outside Supervised, Flow answers for the
...
services *Flow offers the agent*, not permissions the agent needs. `fs` exists so
an editor can serve unsaved buffer contents in place of what is on disk, and
`terminal` lets the agent run commands through the client's own terminal. Flow
...
Advertising a capability Flow cannot service is the harmful choice, because the
...
The one case that would justify serving `fs/read_text_file` is Flow's own file
...
Flow's `InteractionMode` (Build / Plan) and `RuntimeMode` (Supervised /
Auto-accept edits / Auto / Full access) collapse into each CLI's own vocabulary.
Plan always wins over the access mode.

| Flow | Codex (`approvalPolicy` / `sandbox` / reviewer) | Claude `--permission-mode` | Cursor | OpenCode | Grok |
| --- | --- | --- | --- | --- | --- |
...
with the session and is what makes a Flow task outlive its process:
...
| Provider | T3 Code transport | Flow transport |
| --- | --- | --- |
...
streaming-input protocol, which Flow speaks directly. No Node sidecar and no npm
dependency.

Flow goes one further than the comparison: Amp and Pi, which T3 Code does not
support, are long-lived here too. Every provider holds a session.

What the long-lived session buys, and what Flow pays for not having it:

| Capability | T3 Code | Flow |
| --- | --- | --- |
...
`capabilities` record. Flow's equivalent surface is split between
...
Note the parts that are *not* a gap. Flow's Codex path is the same app-server
