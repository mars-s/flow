...
Flow auto-updates with [Sparkle](https://sparkle-project.org). Releases live in
a **Cloudflare R2** bucket served at **`https://releases.flow.sh`**. New users
...
`https://releases.flow.sh/appcast.xml`, verifies each build's EdDSA signature,
...
- Updater code: [`src/updater.rs`](src/updater.rs) — loads the embedded
  Sparkle.framework at runtime and starts `SPUUpdater` with Flow's custom user
...
**This Mac already has the key** — Flow signs with the same default-account
...
Sparkle tools (they land in `.flow-cache/sparkle/<version>/bin` after any
...
To split Flow onto its own key later: `generate_keys --account flow`, put the
new public key in Info.plist, and pass `--account flow` through to
...
profile with `--notary-profile` / `FLOW_NOTARY_PROFILE`.

### 3. Cloudflare R2 bucket + domain  ← **still to do once**

1. Create the bucket **`flow-releases`** (Cloudflare dashboard → R2 → Create
   bucket). The release script will not create it — a bucket-scoped API token
   can't.
2. Attach the custom domain **`releases.flow.sh`** to the bucket (bucket →
   Settings → Custom Domains). This serves objects publicly at
   `https://releases.flow.sh/<file>`.
3. Make sure the R2 API token behind the `r2` rclone remote covers this bucket
   ...
   for kero; if `rclone lsf r2:flow-releases --s3-no-check-bucket` returns
...
- **Download link**: `https://releases.flow.sh/Flow-<version>.dmg`
...
- `flow-<version>-x86_64-unknown-linux-gnu.tar.gz`
- `flow-<version>-aarch64-unknown-linux-gnu.tar.gz`
...
uploads the assets to the `flow-releases` R2 bucket. Configure these repository
secrets first:

| Secret | Purpose |
| --- | --- |
| `FLOW_ANALYTICS_ENDPOINT` | embedded in the macOS CI build |
| `FLOW_ANALYTICS_WEBSITE_ID` | embedded in the macOS CI build |
| `FLOW_SIGNING_IDENTITY` | Developer ID identity selector |
...
| `R2_BUCKET` | optional; defaults to `flow-releases` |
...
| `--build-number <n>` / `FLOW_BUILD_NUMBER` | derived | `CFBundleVersion` override |
| `FLOW_R2_REMOTE` | `r2` | rclone remote name |
| `FLOW_R2_BUCKET` | `flow-releases` | R2 bucket |
| `FLOW_DOWNLOAD_URL_PREFIX` | `https://releases.flow.sh/` | base URL in the appcast |
| `FLOW_HISTORY_COUNT` | `15` | recent archives pulled for delta generation |
| `FLOW_NO_HISTORY=1` | — | skip pulling old archives (full updates only) |
| `SPARKLE_BIN` | the `.flow-cache` copy | Sparkle tools directory |
...
- **Debug builds never update themselves.** `Updater::init` returns `None`
  ...
  itself with a production Flow. Set `FLOW_FORCE_UPDATER=1` to exercise the
  ...
  start the watcher with `FLOW_PREVIEW_UPDATE=1`; the sidebar immediately
...
- **Flow isn't sandboxed**, so Sparkle's XPC services are unnecessary;
...
- **Platform artifacts:** keep the bucket layout flat and platform-tagged by
  ...
  (`Flow-<v>.dmg`, `Flow-<v>.zip`, `appcast.xml`) must keep their URLs.
  Linux CI releases produce `flow-<v>-<target>.tar.gz` with
  ...
  `Flow-<v>-Setup.exe` + `appcast-windows.xml` (WinSparkle reads the same
