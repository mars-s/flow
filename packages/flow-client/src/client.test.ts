...
import { FlowClient, FlowRpcError, daemonUrl, type WebSocketLike } from "./client";
...
async function connect(client: FlowClient, sockets: FakeSocket[]): Promise<FakeSocket> {
...
describe("FlowClient", () => {
  ...
  test("surfaces daemon errors", async () => {
    ...
    await expect(response).rejects.toBeInstanceOf(FlowRpcError);
  });
  ...
  test("disconnect rejects an in-flight handshake and permits reconnecting", async () => {
    ...
    const firstConnection = client.connect();
    client.disconnect();
    await expect(firstConnection).rejects.toThrow("Flow client disconnected");
  ...
  test("disconnected requests reject instead of throwing synchronously", async () => {
    ...
    await expect(request).rejects.toThrow("Flow daemon is disconnected");
  });
  ...
  test("disconnected notifications reject instead of throwing synchronously", async () => {
    ...
    await expect(notification).rejects.toThrow("Flow daemon is disconnected");
  });
...
test("daemonUrl pins the versioned endpoint", () => {
  ...
  expect(daemonUrl("wss://flow.example.test")).toBe("wss://flow.example.test/v1");
});
