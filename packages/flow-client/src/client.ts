...
/** Browser-safe client for Flow's versioned JSON-over-WebSocket protocol. */
export class FlowClient {
  ...
  connect(): Promise<void> {
    ...
    if (this.state === "connecting") {
      return Promise.reject(new Error("Flow client is already connecting"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.addEventListener("message", (event) => {
        ...
        try {
          ...
          failHandshake(new Error("Flow daemon sent invalid JSON"));
          return;
        }
        ...
        if (!handshakeSettled) {
          ...
          failHandshake(new Error("Flow daemon sent an invalid handshake response"));
      ...
      socket.addEventListener("error", () => {
        failHandshake(new Error("Flow daemon connection failed"));
      });
      socket.addEventListener("close", () => {
        if (generation !== this.connectionGeneration) return;
        failHandshake(new Error("Flow daemon disconnected during handshake"));
        this.markDisconnected(new Error("Flow daemon disconnected"));
      });
    });
  }
  ...
  request(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new Error("timed out waiting for Flow daemon"));
  ...
  disconnect(): void {
    this.rejectConnect?.(new Error("Flow client disconnected"));
    ++this.connectionGeneration;
    const socket = this.socket;
    this.socket = undefined;
    this.markDisconnected(new Error("Flow client disconnected"));
  ...
  private requireSocket(): WebSocketLike {
    if (this.state !== "connected" || !this.socket || this.socket.readyState !== OPEN) {
      throw new Error("Flow daemon is disconnected");
    }
    return this.socket;
  }
  ...
  private handleMessage(message: ServerMessage): void {
    if (message.type === "response") {
      ...
      else pending.reject(new FlowRpcError(message.outcome.error.message));
      return;
    }
