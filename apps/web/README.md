...
Browser client for an existing Flow daemon. The Cloudflare Worker serves the
TanStack Start application only; it does not start, proxy, or store credentials
for a daemon.

```sh
bun --filter @flow/web dev
```
...
```sh
FLOW_DAEMON_TOKEN=replace-me cargo run -p flow-daemon --bin flow-daemon -- \
  --bind 127.0.0.1:34123 \
  --allow-origin http://localhost:3001
```

Flow Desktop can expose the daemon it manages from Settings → Daemon, where you
...
```sh
bun --filter @flow/web build
bun --filter @flow/web deploy
```
