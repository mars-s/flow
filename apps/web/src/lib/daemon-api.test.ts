...
import type { ComposerDraftChange, DaemonSettings, Project, FlowClient } from '@flow/client'
import {
...
describe('turn checkpoints', () => {
  test('captures the immutable starting ref on the daemon host', async () => {
    ...
    await expect(captureTurnStart(client, '/srv/flow', 'session', 2)).resolves.toBeUndefined()
    expect(command).toEqual({
      type: 'workspace',
      operation: {
  ...
  test('returns the ending checkpoint captured by the daemon', async () => {
    ...
    const checkpoint = {
      turn_count: 2,
      git_ref: 'refs/flow/session-session-turn-2',
    ...
    await expect(captureTurnCheckpoint(client, '/srv/flow', 'session', 2))
...
describe('probeProvider', () => {
  test('can detect an executable without starting model or version discovery', async () => {
    ...
    await expect(probeProvider(client, 'codex', settings, {
      ...
      probeVersion: false,
    })).resolves.toMatchObject({ installed: true, path: '/opt/flow/codex' })
    expect(command).toEqual({
      type: 'probeProvider',
      provider: 'codex',
      binaryOverride: null,
...
describe('writeWorkspaceTextFile', () => {
  test('writes the edited contents through the daemon workspace API', async () => {
    ...
    await expect(
      writeWorkspaceTextFile(client, '/srv/flow', 'src/app.ts', 'export const ready = true\n'),
...
describe('createProject', () => {
  test('normalizes a remote absolute path without collapsing the root', () => {
    ...
    expect(createProject('/srv/flow/').path).toBe('/srv/flow')
    expect(createProject('/srv/flow/').name).toBe('flow')
  })
...
describe('persistProject', () => {
  test('adds a daemon-host project without creating a session', async () => {
    ...
    const candidate = project('new', 'flow', '/srv/flow')
      ...
      request: async (command: unknown) => {
        ...
        if ((command as { type: string }).type === 'loadTaskState') {
          return {
            ...
            sessions: [{ id: 'session' }],
            defaultCwd: '/srv',
            projectlessRoot: '/srv/.flow/projects',
          }
        }
  ...
  test('reuses a project already persisted for the same daemon path', async () => {
    const existing = project('existing', 'flow', '/srv/flow')
    const commands: unknown[] = []
    const client = {
      request: async (command: unknown) => {
        commands.push(command)
    ...
    const result = await persistProject(client, project('duplicate', 'flow', '/srv/flow'))
...
describe('selectableProjects', () => {
  test('represents projectless tasks as one choice while preserving the selected workspace', () => {
    const ordinary = project('repo', 'flow', '/srv/flow')
    const first = project('one', 'No project', '/home/me/.flow/projects/one')
    const selected = project('two', 'No project', '/home/me/.flow/projects/two')
