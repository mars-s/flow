...
export async function loadTaskState(client: FlowClient): Promise<TaskState> {
  return expectResponse(await client.request({ type: 'loadTaskState' }), 'taskState')
}

export async function loadComposerDrafts(client: FlowClient): Promise<ComposerDrafts> {
...
export async function trashSkills(client: FlowClient, dirs: string[]): Promise<void> {
...
export async function pushWorkspace(client: FlowClient, cwd: string): Promise<void> {
