...
const SESSION_KEY = 'flow.remote.session.v1'
const PERSISTENT_KEY = 'flow.remote.persistent.v1'
...
export function normalizeDaemonAddress(
  ...
  if (
    ...
    throw new Error(t?.('web.error_secure_websocket_required') ?? 'A secure Flow Web page can only connect through wss://')
  }
