...
import { FlowIcon, type FlowIconName } from '@/components/flow-icon'
...
export function ControlMenu({
  ...
  return (
    ...
    >
      ...
      >
        {icon && <FlowIcon className="size-[11px] text-[var(--text-tertiary)]" name={icon} />}
        {children ?? <span className="truncate">{label}</span>}
        {caret && <FlowIcon className="size-2.5 text-[var(--text-ghost)]" name="chevronDown" />}
      </Menu.Trigger>
      <Menu.Portal>
        <Menu.Positioner
        ...
        >
          <Menu.Popup
            aria-label={label}
            className={cn('flow-menu-surface', menuClassName)}
            ...
            {items.map((item, index) => (
              <div key={item.id}>
                {item.separatorBefore && index > 0 && (
                  <div className="flow-menu-separator" role="separator" />
                )}
                ...
                >
                  {item.icon && <FlowIcon className="size-3 text-current" name={item.icon} />}
                  ...
                  {item.selected && <FlowIcon className="size-[11px] text-[var(--text-tertiary)]" name="check" />}
                </Menu.Item>
              </div>
            ))}
          </Menu.Popup>
