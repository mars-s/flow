...
const appName = "Flow";
const executableName = "Flow";
const jsReplExecutableName = "flow_js_repl";
const daemonExecutableName = "flow-daemon";
const computerUseHelperName = "Flow Computer Use";
const packageName = "flow";
const defaultNotaryProfile = "NOTARY";
const projectRoot = resolve(import.meta.dir, "..");

const help = `Build, notarize, and publish a production release of Flow.
...
https://releases.flow.sh. One-time setup lives in RELEASING.md.

Options:
  --local                       Build, notarize, and write the DMG + zip
                                without publishing to R2
  --force                       Publish even if this version is already in R2
  --output <path>               DMG output path (default: dist/Flow-<version>.dmg)
  --signing-identity <name>     Developer ID Application identity selector
                                (or FLOW_SIGNING_IDENTITY; required unless --adhoc)
  --notary-profile <name>       notarytool keychain profile
                                (default: NOTARY; or FLOW_NOTARY_PROFILE)
  --build-number <number>       CFBundleVersion override (or FLOW_BUILD_NUMBER;
                                default derives a monotonic number from the
                                Cargo version)
  --volume-name <name>          Mounted DMG name (default: Flow)
  --skip-build                  Reuse target/release/flow, flow_js_repl, and
                                flow-daemon
...
Environment:
  FLOW_SIGNING_IDENTITY         Developer ID Application identity selector
  FLOW_ANALYTICS_ENDPOINT       analytics endpoint embedded at build time
  FLOW_ANALYTICS_WEBSITE_ID     analytics website ID embedded at build time
  FLOW_R2_REMOTE                rclone remote name (default: r2)
  FLOW_R2_BUCKET                R2 bucket name (default: flow-releases)
  FLOW_DOWNLOAD_URL_PREFIX      base URL served by the bucket
                                (default: ${defaultDownloadUrlPrefix})
  FLOW_HISTORY_COUNT            prior archives pulled for deltas (default: 15)
  FLOW_NO_HISTORY=1             skip pulling prior archives (no deltas)
  SPARKLE_BIN                   Sparkle tools dir (default: the bundle.sh cache
                                under .flow-cache/sparkle)
...
const configuredSigningIdentity =
  values["signing-identity"] ?? process.env.FLOW_SIGNING_IDENTITY;
const notaryProfile =
  values["notary-profile"] ??
  process.env.FLOW_NOTARY_PROFILE ??
  defaultNotaryProfile;
const explicitBuildNumber =
  values["build-number"] ?? process.env.FLOW_BUILD_NUMBER;
const analyticsEndpoint = process.env.FLOW_ANALYTICS_ENDPOINT?.trim();
const analyticsWebsiteId = process.env.FLOW_ANALYTICS_WEBSITE_ID?.trim();
...
const r2Remote = process.env.FLOW_R2_REMOTE ?? "r2";
const r2Bucket = process.env.FLOW_R2_BUCKET ?? "flow-releases";
...
const historyCount = Number(process.env.FLOW_HISTORY_COUNT ?? "15");
const skipHistory = process.env.FLOW_NO_HISTORY === "1";
...
if (!adhoc && !configuredSigningIdentity) {
  throw new Error(
    "Set FLOW_SIGNING_IDENTITY or pass --signing-identity (or use --adhoc).",
  );
}
...
if (!Number.isSafeInteger(historyCount) || historyCount < 0) {
  throw new Error("FLOW_HISTORY_COUNT must be a non-negative integer.");
}
if (!values["skip-build"] && (!analyticsEndpoint || !analyticsWebsiteId)) {
  throw new Error(
    "Set FLOW_ANALYTICS_ENDPOINT and FLOW_ANALYTICS_WEBSITE_ID before building a release.",
  );
}
    ...
    if (detail.includes("directory not found")) {
      throw new Error(
        `R2 bucket "${r2Bucket}" does not exist on remote "${r2Remote}". ` +
          ...
          "releases.flow.sh custom domain (see RELEASING.md), then re-run.",
      );
    }
  ...
  const requests = [
    {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "flow-release", version: "1" },
      },
    },
...
try {
  ...
  await $`env FLOW_CODESIGN_IDENTITY=${identity} FLOW_ANALYTICS_ENDPOINT=${analyticsEndpoint ?? ""} FLOW_ANALYTICS_WEBSITE_ID=${analyticsWebsiteId ?? ""} FLOW_SKIP_CARGO_BUILD=${values["skip-build"] ? "1" : "0"} ${join(projectRoot, "scripts", "bundle.sh")} release`;
  ...
  temporaryDirectory = await mkdtemp(join(tmpdir(), "flow-dmg-"));
  ...
  // archive as Flow-<version>.md; generate_appcast links it as the update's
  ...
  if (publishing) {
    ...
    console.log(`\nFlow ${version} (build ${buildNumber}) is live:`);
